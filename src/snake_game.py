...
class SnakeGame(Tk):
    ...
    def h(self, n, snake=None):
        ...
        return float(h_cols+h_rows) * self.bias[0] + float(rect_a-len(snake)) * self.bias[1] + h_wall * self.bias[2] + h_free * self.bias[3] + float(h_edge) * self.bias[4]
    ...
    def gameover(self):
        ...
        self.head = 22
        self.snake = [self.head,self.head+self.width,self.head+(2*self.width)]
        self.food = [randint(0, self.height*self.width-1)]
    ...
    def setBias(self, bias):
        ...
        for i in range(len(self.bias)-1):
            self.key += str(self.bias[1]) + ':'
        self.key += str(self.bias[len(self.bias)-1])
